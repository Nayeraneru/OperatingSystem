...
void env_free(struct Env *e)
{
	...
	if (!e || !e->env_page_directory)
	    return;

	// Unmap and free pages in the working set or LRU lists
	if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
	{
	    // Process both ActiveList and SecondList for LRU replacement
	    struct WorkingSetElement *currentElement = NULL;

	    // Handle ActiveList
	    while (!LIST_EMPTY(&(e->ActiveList)))
	    {
	        currentElement = LIST_FIRST(&(e->ActiveList));
	        if (currentElement)
	        {
	        	/*uint32* ptr_page_table;
	            struct FrameInfo* ptrFrm = get_frame_info(e->env_page_directory, currentElement->virtual_address, &ptr_page_table);
	        	free_frame(ptrFrm);*/
	            unmap_frame(e->env_page_directory, currentElement->virtual_address);
	            LIST_REMOVE(&(e->ActiveList), currentElement);
	            kfree(currentElement);
	        }
	    }
	    ...
	    {
	        currentElement = LIST_FIRST(&(e->SecondList));
	        if (currentElement)
	        {
	        	/*uint32* ptr_page_table;
	            struct FrameInfo* ptrFrm = get_frame_info(e->env_page_directory, currentElement->virtual_address, &ptr_page_table);
	        	free_frame(ptrFrm);*/
	            unmap_frame(e->env_page_directory, currentElement->virtual_address);
	            LIST_REMOVE(&(e->SecondList), currentElement);
	            kfree(currentElement);
	        }
	    }
	}
	...
	{
	    // Handle environments not using LRU replacement
	    ...
	    while (!LIST_EMPTY(&(e->page_WS_list)))
	    {
	        ...
	        {
				/*uint32* ptr_page_table;
				struct FrameInfo* ptrFrm = get_frame_info(e->env_page_directory, wsElement->virtual_address, &ptr_page_table);
				free_frame(ptrFrm);*/
	            unmap_frame(e->env_page_directory, wsElement->virtual_address);
	...
	// Free all page tables in the virtual memory range
	for (uint32 virtualAddress = USER_HEAP_START; virtualAddress < USER_HEAP_MAX; virtualAddress += (PAGE_SIZE * 1024))
	{
	    uint32 *pageTable = NULL;

	    // Check if the page table exists in memory
	    if (get_page_table(e->env_page_directory, virtualAddress, &pageTable) == TABLE_IN_MEMORY)
	    {
	        kfree(pageTable);                       // Free the allocated page table
	        pd_clear_page_dir_entry(e->env_page_directory, virtualAddress);  // Invalidate directory entry
	    }
	}
	...
	if (e->env_page_directory)
	{
	    ...
	    e->env_page_directory = NULL; // Ensure the pointer is cleared
	}
	e->disk_env_pgdir_PA = 0; // Clear any associated disk data

    // Free the user kernel stack
    delete_user_kern_stack(e);

    // Free Shared Objects (semaphores too)
    struct Share *shared_element = NULL;
    LIST_FOREACH(shared_element, &AllShares.shares_list)
    {
        if (shared_element->ownerID == e->env_id)
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	struct BlockElement* kstack_start = kmalloc(KERNEL_STACK_SIZE);
	if (kstack_start == NULL)
		panic("Unable to allocate stack!");
	uint32* table_ptr = NULL;
	uint32 gaurd_page = (uint32)kstack_start;
	get_page_table(ptr_user_page_directory,gaurd_page,&table_ptr);
	(table_ptr[PTX(gaurd_page)]) &= ~PERM_PRESENT;
	return kstack_start;
...
void delete_user_kern_stack(struct Env* e)
{
    ...
    if (e == NULL || e->kstack == NULL)
        return;

    // Calculate the virtual address of the guard page
    uint32 guardPageVA = (uint32)e->kstack;

    // Calculate the virtual address of the stack's top page
    uint32 stackTopVA = guardPageVA + (e->initNumStackPages + 1) * PAGE_SIZE;

    // Iterate through all pages (including the guard page) and unmap them
    for (uint32 va = guardPageVA; va < stackTopVA; va += PAGE_SIZE)
    {
        // Unmap the frame associated with this virtual address
        unmap_frame(ptr_page_directory, va);

        // Clear the page permissions
        pt_set_page_permissions(ptr_page_directory, va, 0, PERM_WRITEABLE | PERM_USER | PERM_PRESENT);
    }

    // Free the stack memory region from the kernel heap
    kfree(e->kstack);
