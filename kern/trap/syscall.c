...
uint32 get_page_permissions(uint32 virtual_address){

	uint32 perm = pt_get_page_permissions(cur_env->env_page_directory, virtual_address);
			if(perm == -1)
				perm = 0;
...
void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	if(virtual_address >= USER_HEAP_START && virtual_address <= USER_HEAP_MAX){
		if((virtual_address+size) >= USER_HEAP_START && (virtual_address+size) <= USER_HEAP_MAX){
			...
			cprintf("Before exit 1 \n");
			env_exit();
		}
	}else {
		cprintf("Before exit 2 \n");
		env_exit();
	}
...
//TODO: [PROJECT'24.MS1 - #03] [2] SYSTEM CALLS - Params Validation
    //cprintf("Calling MS1 sys_allocate_user_mem \n");
...
int sys_queue_init(struct Env_Queue* queue) {
...
void sys_enqueue_queue(struct semaphore* sem) {
...
struct Env* sys_cur_proc(void){
	struct Env* env = get_cpu_proc();

	return env;
}

void sys_ready_queue(struct Env_Queue* env_ready_queues) {
	acquire_spinlock(&ProcessQueues.qlock);

	//struct Env* env = get_cpu_proc();
	struct Env* dequeue_env = dequeue(env_ready_queues);
	//cprintf("sys ready queue semaphoreee\n");
	//dequeue_env->env_status = ENV_READY;
	sched_insert_ready0(dequeue_env);
	//cprintf("env ready in sys ready queue semaphoreee\n");
	//sched();
	release_spinlock(&ProcessQueues.qlock);
	//cprintf("after env ready in sys queue semaphoreee\n");
}
...
{
	return get_page_permissions( virtual_address);
}
/*********************************/
...
{
	env_set_priority((int)envID, priority);
}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_ready_queue:
		sys_ready_queue((struct Env_Queue*)a1);
		break;
	case SYS_cur_proc:
		return (int)sys_cur_proc();
		break;
	case SYS_enqueuee_queue:
		sys_enqueue_queue((struct semaphore *)a1);
		return 0;
		break;
	case SYS_queue_init:
		return (int)sys_queue_init((struct Env_Queue*)a1);
		break;
	...
	case SYS_get_page_permission:
		return sys_get_page_permissions(a1);
	    break;
	//======================================================================
	case SYS_cputs:
