...
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum, uint32 starvThresh)
{
	...
	//panic("Not implemented yet");
	sched_delete_ready_queues();
	init_spinlock(&ProcessQueues.qlock, "secure processes queues");
	STARVATION_THRESHOLD = starvThresh;
//	cprintf("starvation set\n");
	num_of_ready_queues= numOfPriorities;
//	cprintf("quantum set=%d\n",quantums[0] );
//	cprintf("periority set=%d\n",numOfPriorities );
//	cprintf("quantum set=%d\n",quantums[0] );
//	cprintf("starv set=%d\n",starvThresh );
...
{
	/*To protect process Qs (or info of current process) in multi-CPU******************olll******/
	if(!holding_spinlock(&ProcessQueues.qlock))
	...
	if (cur_env != NULL) {
			...
			if (priority < 0 || priority >= num_of_ready_queues) {
				panic("fos_scheduler_PRIRR: Invalid priority for current environment.");
			}
			cur_env->tick_counter = timer_ticks();
...
//	for(int i = 0; i < num_of_ready_queues; i++) // Skip the highest-priority queue
//		{
//			int size = queue_size(&(ProcessQueues.env_ready_queues[i]));
//			if (size >0) {
//				struct Env_Queue* queue = &(ProcessQueues.env_ready_queues[i]);
//				next_env = LIST_FIRST(queue);
//				break;
//			}
//		}

	// If no process is found, you can return NULL or an idle environment
	if (next_env == NULL) {
		return NULL;
	}
	if (next_env) {
		next_env->tick_counter =0;// Initialize tick counter for the selected environment
	} //timer_ticks()
	kclock_set_quantum(quantums[0]);
//	cprintf("return new env\n");
	return next_env;
}
//struct Env* fos_scheduler_PRIRR()
//{
...
//		struct Env *next_env = NULL;
//		struct Env *cur_env = get_cpu_proc();
//		cprintf("hello to processes world\n");
//		/*//If the curenv is still exist, then insert it again in the ready queue
//		if (cur_env != NULL)
//		{
//			enqueue(&(ProcessQueues.env_ready_queues[0]), cur_env);
//		}*/
//
//	    // If there's a current process, enqueue it in the appropriate ready queue based on its priority
//	    if (cur_env != NULL) {
//	        int priority = cur_env->priority;
//	        if (priority < 0 || priority >= num_of_ready_queues) {
//	            panic("fos_scheduler_PRIRR: Invalid priority for current environment.");
//	        }
//	        cprintf("set the proc to suitable queue\n");
...
//		next_env = dequeue(&(ProcessQueues.env_ready_queues[0]));*/
//
//	    // Iterate through priority queues (highest to lowest) to find the next process to run
//	    for (int i = 0; i < num_of_ready_queues; i++) {
//	        if (!LIST_EMPTY(&(ProcessQueues.env_ready_queues[i]))) {
//	        	cprintf("find non empty env queue here \n");
//	            next_env = dequeue(&(ProcessQueues.env_ready_queues[i]));
//	            cprintf("new env here \n");
//	            break;
//	        }
//	    }
//
...
//		//cprintf("CLOCK INTERRUPT AFTER RESET: Counter0 Value = %d\n", cnt0 );
//
//	    // Set the quantum for the selected process based on its priority
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
        ...
        acquire_spinlock(&ProcessQueues.qlock); // Acquire lock to protect process queues

        for (int priority = num_of_ready_queues-1; priority >=1; priority--) // Skip the highest-priority queue
		        {
		            struct Env_Queue* queue = &(ProcessQueues.env_ready_queues[priority]);
		            struct Env* env = LIST_FIRST(queue);

		            while (env != NULL)
		            {
		                struct Env* next_env = LOOP_LIST_NEXT(env);

//		                if ((((ticks-env->tick_counter)%STARVATION_THRESHOLD)==0)&& (env->checkedEnv==0))
		                if (((env->tick_counter%STARVATION_THRESHOLD)==0)&& (env->checkedEnv==0))
		                {
//		                    env->tick_counter = timer_ticks(); // Reset the tick counter
		                    // Promote the process to a higher priority queue
//		                	cprintf("starv set=%d\n",STARVATION_THRESHOLD );
//		                	cprintf("ticks set=%d\n",ticks );
//		                	cprintf("env ticks set=%d\n",env->tick_counter );
//		                	cprintf("env id set=%d\n",env->env_id );
		                    int new_priority = priority - 1;
		                    if (new_priority < 0)
		                    {
		                        new_priority = 0; // Ensure it does not exceed the highest priority
		                    }
		                    env->checkedEnv=1;
		                    env_set_priority(env->env_id,new_priority );
		                }
		                env = next_env; // Move to the next environment in the list
		            }
		        }
        release_spinlock(&ProcessQueues.qlock); // Release the lock
	}
	...
	/********DON'T CHANGE THESE LINES***********/
	ticks++ ;
	acquire_spinlock(&ProcessQueues.qlock); // Acquire lock to protect process qu
	for (int priority = 0; priority <num_of_ready_queues ; priority++) // Skip the highest-priority queue
	{
		struct Env_Queue* queue = &(ProcessQueues.env_ready_queues[priority]);
		struct Env* env = LIST_FIRST(queue);
		while (env != NULL)
