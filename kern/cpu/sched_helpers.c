...
void sched_run_all()
{
//	cprintf("enter run all function\n");
	acquire_spinlock(&(ProcessQueues.qlock)); 	//CS on Qs
	struct Env* ptr_env=NULL;
...
//		cprintf("dequeue\n");
		sched_insert_ready(ptr_env);
//		cprintf("insert ready\n");
		sched_print_all();
	}
//	cprintf("done insertion\n");
	release_spinlock(&(ProcessQueues.qlock)); 	//CS on Qs
//	cprintf("release lock\n");
	/*2015*///if scheduler not run yet, then invoke it!
	if (mycpu()->scheduler_status == SCH_STOPPED){
//		cprintf("sched stopped condition\n");
		fos_scheduler();
...
void env_set_priority(int envID, int priority)
{
	...
	int prev_periority= proc->priority; //save previos one
	 //update periority
	if((proc->env_status==ENV_READY) && (prev_periority!=priority)){ //if priority is the same, env will not be rescheduled
		sched_remove_ready(proc);
		proc->priority= priority;
		sched_insert_ready(proc);
	}else if ((prev_periority!=priority)) proc->priority= priority;
	if (!lock_already_held)
		{
			release_spinlock(&ProcessQueues.qlock);
		}
