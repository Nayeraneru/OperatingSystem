...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands); //empty the list first
	//check if command if found in the command list

	int found=0; //act as a flag
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		if (strcmp(arguments[0], commands[i].name) == 0) //if cmd matched, will enter if condition
		{
			//found and correct num of args=> for commands which accept zero or more args
			if(((number_of_arguments-1)== commands[i].num_of_args) &&(commands[i].num_of_args >=0))
			{
				return i;
			} //at least one arg commands condition
			else if(((number_of_arguments-1)>commands[i].num_of_args)&&((number_of_arguments-1)>0)&&(commands[i].num_of_args ==-1))
			{
				return i;
			}
			else { //found but wrong num of args
				LIST_INSERT_TAIL(&foundCommands, &commands[i]); //push the found command
				return CMD_INV_NUM_ARGS;
			}
		}
	}

	found =-1; //flag that command not found by name provided
	char *foundChar;
	int matchedChars=0;
	int matching[40];
	int flag=0;
	if(found ==-1){
		for (int i = 0; i < NUM_OF_COMMANDS; i++)
		{
			for(int j=0; j< strlen(arguments[0]);j++){
				foundChar= strfind(commands[i].name, arguments[0][j]); //check if char exists
				if(*foundChar == arguments[0][j]){
					matching[matchedChars]=(int)foundChar; //save the arrangment of chars found
					matchedChars++;
				}
			}
			if(matchedChars==strlen(arguments[0])){
				if(strlen(arguments[0])>1){
					for(int k=0;k<matchedChars-1;k++){
						if(matching[k]>matching[k+1]){ //check if chars found in the correct order
							flag=1;//not subsequent
							break;
						}
					}
				}
			...
			flag=0;
			matchedChars=0; //zeroing count
			//array is not initialized again, we overwrite its indecies
		}
		if(found==1){
			return CMD_MATCHED;
		}
